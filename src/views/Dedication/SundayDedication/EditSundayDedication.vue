      ...
      >
        <div class="setting">
          <div class="search" id="sear_db">
            <a><a-icon type="environment" style="margin-right: 3px"/>編輯主日奉獻</a>
          </div>
          <div class="userNum">
            最後更新時間： {{ updated_at }}
          </div>
        ...
        <div class="box_content">
          <div class="box_content_top">
            ...
            <div class="bar">
              <i style="margin-right:15px"></i><a>姓名：</a>
              <a-input class="inputBox" v-model="name" disabled/>
...
export default {
  ...
  methods: {
    ...sundayDedication.mapActions({
      // 儲存主日奉獻
      getSaveRegularDonationsData: 'getSaveRegularDonationsData',
    }),
    ...
    // 上一頁
    forwardup() {
      this.$router.push("/SundayDedication");
    },
    // 選擇奉獻日期
    ...
    addUser() {
      if (this.home_number == "" || this.donation_at == "" || this.donation_amount == "") {
        ...
        this.loading = true
        this.getSaveRegularDonationsData({
          _id: this.id,
          ...
          Modal.success({
            ...
            okText: '確認',
            onOk() {
              vs.$router.push("/SundayDedication");
            }
          });
        }).catch(error => {
          this.loading = false
