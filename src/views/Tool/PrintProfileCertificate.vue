      ...
      >
        <div class="setting">
          <div class="search" id="sear_db">
            <a><a-icon type="environment" style="margin-right: 3px"/>列印領洗堅振證明</a>
...
export default {
  ...
  methods: {
    ...report.mapActions({
      // 列印領洗證明
      printProfileCertificate: 'printProfileCertificate',
    }),
    ...
    exportProfileData() {
      if (this.selectedRowKeys == "") {
        ...
        let _id = this.selectedRowKeys[0]
        let name = this.data.filter(x => x.id == _id)[0].name
        // for (let i = 0; i < this.selectedRowKeys.length; i++) {
        //   ids.push(this.selectedRowKeys[i])
        // }
        setTimeout(() => {
          this.loading = false
          this.printProfileCertificate({
            _id: _id
          }).then(res => {
            // console.log("309",res);
            // 自動下載
            ...
            link.href = docxUrl;
             // 指定下載的檔案名稱
            link.setAttribute('download', `${name}-領洗堅振證明書.docx`);
