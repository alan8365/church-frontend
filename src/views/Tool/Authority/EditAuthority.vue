      ...
      >
        <div class="setting">
          <div class="search" id="sear_db">
            <a><a-icon type="environment" style="margin-right: 3px"/>編輯管理員資料</a>
          </div>
          <div class="userNum">
            最後更新時間： {{ updated_at }}
          </div>
        ...
        <div class="box_content">
          <div class="box_content_top">
            <div class="bar">
              <i style="margin-left: 30px">*</i><a>帳號：</a>
              <a-input class="inputBox" placeholder="請輸入帳號" v-model="username" disabled/>
            </div>
            <div class="bar">
              <i>*</i><a>修改姓名：</a>
            ...
            <div class="bar">
              <!-- <i>*</i> -->
              <a style="margin-left: 10px">修改密碼：</a>
          ...
          <div class="box_content_bottom">
            ...
            <a-button type="primary" @click="saveUser" class="button" style="margin-right: 10px">儲存資料</a-button>
          </div>
        </div>
       </a-col>
    </a-row>
...
export default {
  ...
  methods: {
    ...user.mapActions({
      // 儲存管理員
      getSaveUserData: 'getSaveUserData',
    }),
    // 上一頁
    forwardup() {
    ...
    checkLSEditData() {
      ...
      if (data !== null && data !== undefined) {
        ...
        this.username = JSON.parse(localStorage.getItem("editData")).username
        // this.password = JSON.parse(localStorage.getItem("editData")).password_digest
        this.authority = JSON.parse(localStorage.getItem("editData")).is_admin == true ? 'admin' : JSON.parse(localStorage.getItem("editData")).is_modulator == true ? 'editor' : 'viewer'
        // 最後更新時間
    ...
    // 儲存管理員
    saveUser() {
      if (this.name == "" || this.username == "" || this.authority == "") {
        Modal.warning({
          title: '系統提示',
          content: `請檢查所輸入的資料內容`,
          okText: '確認',
