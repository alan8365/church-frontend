      ...
      >
        <div class="setting">
          <div class="search" id="sear_db">
            <a><a-icon type="environment" style="margin-right: 3px"/>列印教友證</a>
...
export default {
  ...
  methods: {
    ...report.mapActions({
      // 列印教友證
      printParishioner: 'printParishioner',
    }),
    ...
    exportProfileData() {
      if (this.selectedRowKeys == "") {
        ...
        Modal.confirm({
          title: '系統提示',
          content: `目前尚未勾選任何教友資料，是否要列印「全部」教友的教友證？`,
          okText: '匯出全部',
          cancelText: '取消匯出',
          onOk() {
            vs.loading = true
            setTimeout(() => {
              ...
              vs.printParishioner({
                ...
                link.href = pdfUrl;
                // 指定下載的檔案名稱
                link.setAttribute('download', '教友證.pdf');
