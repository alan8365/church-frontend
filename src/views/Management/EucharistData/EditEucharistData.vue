      ...
      >
        <div class="setting">
          <div class="search" id="sear_db">
            <a><a-icon type="environment" style="margin-right: 3px"/>編輯初領聖體資料</a>
          </div>
          <div class="userNum">
            最後更新時間： {{ updated_at }}
          </div>
        ...
        <div class="box_content">
          <div class="box_content_top">
            <div class="box_content_left">
                <!-- <div class="bar">
                    <a>初領聖體編號：</a>
                    <a-input class="inputBox" placeholder="初領聖體編號" v-model="userNum" disabled/>
                </div> -->
                <!-- <div class="bar">
                    <i>*</i><a>家號：</a>
                ...
                <div class="bar">
                    <i>*</i><a>初領聖體教堂：</a>
                    <a-input class="inputBox" placeholder="請輸入初領聖體教堂" v-model="eucharist_location"/>
                </div>
            </div>
            ...
            <div class="box_content_right">
              <div class="bar">
                  <i>*</i><a>初領聖體日期：</a>
                  <a-date-picker @change="onChangeEucharistDate" placeholder="請選擇初領聖體日期" v-model="eucharist_at"/>
                   <div class="searchButton" :style="{ width: scroll_X > 550 ? '0px' : '10px'}"></div>
              </div>
                <div class="bar">
                    <i>*</i><a>初領聖體司鐸：</a>
                    <a-input class="inputBox" placeholder="請輸入初領聖體司鐸" v-model="presbyter"/>
                     <div class="searchButton" :style="{ width: scroll_X > 550 ? '0px' : '10px'}"></div>
                </div>
                <div class="bar">
...
const eucharist = createNamespacedHelpers('eucharist');
...
export default {
  ...
  methods: {
    ...
    ...eucharist.mapActions({
      // 儲存初領聖體資料
      getSaveEucharistData: 'getSaveEucharistData',
    }),
    // 上一頁
    forwardup() {
      this.$router.push("/EucharistData");
    },
    // 選擇初領聖體日期
    onChangeEucharistDate(date, dateString) {
      // console.log(date, dateString);
      this.eucharist_at = dateString
    },
    // 選擇代父、代母
    ...
    checkAllPresbyter() {
      ...
      this.getAllParishioners().then(res => {
        ...
        // 過濾為領洗的教友
        let data = res.data.filter(x => x.eucharist == undefined)
        if (data !== 0) {
    ...
    checkLSEditData() {
      ...
      if (data !== null && data !== undefined) {
        ...
        this.eucharist_location = JSON.parse(localStorage.getItem("editData")).eucharist_location
        ...
        this.eucharist_at = JSON.parse(localStorage.getItem("editData")).eucharist_at
    ...
    // 儲存初領聖體資料
    addUser() {
      if (this.parishioners == "" || this.christian_name == "" || this.eucharist_location == "" || this.presbyter == "" || this.eucharist_at == "" || (this.godfather == "" && this.godmother == "")) {
        Modal.warning({
          title: '系統提示',
          content: `請檢查所輸入的資料內容`,
          okText: '確認',
        ...
        this.loading = true
        this.getSaveEucharistData({
          _parishioner_id: this.parishioners,
          parishioner_id: this.parishioners,
          eucharist_at: this.eucharist_at,
          eucharist_location: this.eucharist_location,
