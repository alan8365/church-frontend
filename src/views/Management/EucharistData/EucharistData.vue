      ...
      >
        <div class="setting">
          <div class="search" id="sear_db">
            <a><a-icon type="environment" style="margin-right: 3px"/>初領聖體</a>
            <input
              class="box_search"
              @change="onSearch"
              placeholder="輸入初領聖體資料"
              v-model="searchText"
            />
          ...
          <div class="crud">
            <router-link
              ...
              to="/EucharistData/AddEucharistData"
              class="crud_button"
              >新增</router-link
            >
          </div>
...
export default {
    ...
    columns() {
      ...
      if (this.user_authority !== '' && this.user_authority !== '檢視者') {
        columns = [
          ...
          //   filters: [
          //     { text: '男', value: 'boy' },
          //     { text: '女', value: 'females' },
          //     { text: '未知', value: 'unknown' },
          //   ],
          //   sorter: (a, b) => a.gender.length - b.keyName.length,
          ...
          {
            title: '初領聖體教堂',
            dataIndex: 'eucharist_location',
            key: 'eucharist_location',
            width: 120,
            sorter: (a, b) => a.eucharist_location.length - b.eucharist_location.length,
            sortOrder: sortedInfo.columnKey === 'eucharist_location' && sortedInfo.order,
          },
          {
            title: '初領聖體司鐸',
            dataIndex: 'presbyter',
  ...
  methods: {
    ...eucharist.mapActions({
      // 撈初領聖體資料
      getAllEucharist: 'getAllEucharist',
      // 刪除初領聖體資料
      getDeleteEucharist: 'getDeleteEucharist',
      // 撈初領聖體資料詳情
      getEucharistData: 'getEucharistData',
      // 搜尋初領聖體資料
      getSearchEucharistData: 'getSearchEucharistData',
    }),
    forwardup() {
          ...
          if (res.data.length !== 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.data.push({
                ...
                eucharist_at: res.data[i].eucharist_at,
                eucharist_location: res.data[i].eucharist_location,
    ...
    checkDeleteUser(christian_name, parishioner_id) {
        ...
        Modal.confirm({
          title: '系統提示',
          content: `請確認是否刪除「${christian_name}」教友初領聖體資料`,
          okText: '確認',
          cancelText: '取消',
          onOk() {
            vs.loading = true
