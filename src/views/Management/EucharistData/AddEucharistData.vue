      ...
      >
        <div class="setting">
          <div class="search" id="sear_db">
            <a><a-icon type="environment" style="margin-right: 3px"/>新增初領聖體資料</a>
          </div>
        </div>
        <div class="box_content">
          <div class="box_content_top">
            ...
            <div class="box_content_right">
              <div class="bar">
                  <i>*</i><a>初領聖體日期：</a>
                  <a-date-picker @change="onChangeEucharistDate" placeholder="請選擇日期" v-model="eucharist_at"/>
...
export default {
  ...
  methods: {
    ...
    ...eucharist.mapActions({
      // 新增初領聖體資料
      getAddEucharist: 'getAddEucharist',
    }),
    // 上一頁
    forwardup() {
        ...
        if (data !== 0) {
          for (let i = 0; i < data.length; i++) {
            this.parishionersArr.push({
              label: data[i].last_name !== null && data[i].first_name !== null ? data[i].last_name + data[i].first_name : "",
              value: data[i].id
            })
          }
        }
    ...
    // 新增堅振資料
    addUser() {
      if (this.parishioners == "" || this.eucharist_location == "" || this.presbyter == "" || this.eucharist_at == "" || (this.godfather == "" && this.godmother == "")) {
        Modal.warning({
          title: '系統提示',
          content: `請檢查所輸入的資料內容`,
          okText: '確認',
