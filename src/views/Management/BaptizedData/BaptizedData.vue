      ...
      >
        <div class="setting">
          <div class="search" id="sear_db">
            <a><a-icon type="environment" style="margin-right: 3px"/>領洗資料</a>
            <input
              class="box_search"
              @change="onSearch"
              placeholder="輸入領洗資料"
          ...
          <div class="crud">
            <router-link
              ...
              to="/BaptizedData/AddBaptizedData"
              class="crud_button"
              >新增</router-link
            >
          </div>
...
export default {
    ...
    columns() {
      ...
      if (this.user_authority !== '' && this.user_authority !== '檢視者') {
        columns = [
          ...
          {
            title: '領洗教堂',
            dataIndex: 'baptized_location',
            key: 'baptized_location',
            width: 150,
            sorter: (a, b) => a.baptized_location.length - b.baptized_location.length,
            sortOrder: sortedInfo.columnKey === 'baptized_location' && sortedInfo.order,
          },
          {
            title: '領洗司鐸',
            dataIndex: 'presbyter',
  ...
  methods: {
    ...baptism.mapActions({
      // 撈領洗資料
      getAllBaptism: 'getAllBaptism',
      // 刪除領洗資料
      getDeleteBaptism: 'getDeleteBaptism',
      // 撈領洗資料詳情
      getBaptismData: 'getBaptismData',
      // 搜尋領洗資料
      getSearchBaptismData: 'getSearchBaptismData',
    }),
    // 自訂性別排序 
    customSort(a, b) {
      // 自訂排序函式的邏輯
          ...
          if (res.data.length !== 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.data.push({
                  ...
                  baptized_at: res.data[i].baptized_at,
                  baptized_location: res.data[i].baptized_location,
                  presbyter: res.data[i].presbyter,
                  christian_name: res.data[i].christian_name,
    ...
    // 編輯教友
    editUser(christian_name, parishioner_id) {
      this.loading = true
      this.getBaptismData({
          ...
          this.$router.push("/BaptizedData/EditBaptizedData");
      }).catch(error => {
          this.loading = false
          Modal.error({
            title: '系統提示',
